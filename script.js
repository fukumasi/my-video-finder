document.addEventListener('DOMContentLoaded', function() {
    ...
    loginForm.addEventListener('submit', function(event) {
        ...
        if (username === validUsername && password === validPassword) {
            ...
            loginForm.style.display = 'none'; // ログインフォームを非表示にする
        } else {
    ...
    function generateRecommendations(history) {
        ...
        const sampleVideos = [
            { category: 'cooking', title: 'Recommended Video 1: Advanced Cooking Techniques' },
            { category: 'technology', title: 'Recommended Video 2: JavaScript Basics' },
            { category: 'cooking', title: 'Recommended Video 3: Gourmet Recipes' },
            { category: 'technology', title: 'Recommended Video 4: Understanding CSS' }
        ];

        sampleVideos.forEach(video => {
            if (history.includes(video.category)) {
                const listItem = document.createElement('li');
